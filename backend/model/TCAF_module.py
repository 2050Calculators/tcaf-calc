...
def _cy_to_t_dm(arr_cy, ref_dm, name):
    ...
    dm = DataMatrix.based_on(
        ...
        format=ref_dm,
        change={'Variables': [name], 'Categories1': None,
                'Categories2': None, 'Categories3': None},
...
def _sum_asf_total(arr_gw, ref_ch_dm):
    ...
    """
    dm = DataMatrix.based_on(
        arr_gw[:, :, np.newaxis, :, :], format=ref_ch_dm,
...
# ---------------------------------------------------------------------------
def TCAF_lca_workflow(DM_TCAF_lca, DM_crop_to_TCAF, DM_landuse_to_TCAF,
                      DM_livestock_to_TCAF, CDM_const, CDM_MF,
                      years_setting=None, dm_cal_ghg_lca=None,
                      cdm_ghg_ef_perhead=None, cdm_lsu_per_head=None,
    ...
    DM_livestock_to_TCAF['asf-world'].drop(col_label=['Switzerland', 'Tokelau'], dim='Country')
    ...
    dm_match_countries(DM_livestock_to_TCAF['asf-world'], DM_TCAF_lca['lca-world'], parameter='perfect match')
    dm_match_countries(DM_livestock_to_TCAF['meat-world'], DM_TCAF_lca['lca-world'], parameter='perfect match')
    ...
    array_temp = 10 ** 3 * DM_crop_to_TCAF[:, :, 'agr_domestic-production_afw', :] \
                 / cdm_kcal[np.newaxis, np.newaxis, 'cp_kcal-per-t', :]
    DM_crop_to_TCAF.add(array_temp, dim='Variables',
                        col_label='agr_domestic-production_afw_kg', unit='kg')
    ...
    DM_landuse_to_TCAF['prod-ch'].groupby({'crop-cereal': 'crop-cereal|crop-rice'},
                                          dim='Categories2', inplace=True, regex=True)
    DM_crop_to_TCAF.groupby({'crop-cereal': 'crop-cereal|crop-rice'},
                            dim='Categories1', inplace=True, regex=True)
    ...
    DM_landuse_to_TCAF['prod-ch'].rename_col('agr_domestic-production_afw_kg', 'agr_production-lca', dim='Variables')
    DM_crop_to_TCAF.rename_col('agr_domestic-production_afw_kg', 'agr_production-lca', dim='Variables')
    # Meat impacts are per kg LIVEWEIGHT -> use liveweight, not edible weight.
    DM_livestock_to_TCAF['meat-ch'].rename_col('liveweight-production', 'agr_production-lca', dim='Variables')
    DM_livestock_to_TCAF['meat-world'].rename_col('liveweight-production', 'agr_production-lca', dim='Variables')
    # Egg/milk impacts are per kg of product -> use the product mass.
    DM_livestock_to_TCAF['asf-ch'].rename_col('agr_domestic_production_liv_afw_kg', 'agr_production-lca', dim='Variables')
    DM_livestock_to_TCAF['asf-world'].rename_col('agr_domestic_production_liv_afw_kg', 'agr_production-lca', dim='Variables')
    ...
    dm_lca_world.append(DM_livestock_to_TCAF['asf-world'].filter({'Variables': ['agr_production-lca']}), dim='Categories1')
    dm_lca_world.append(DM_crop_to_TCAF.filter({'Variables': ['agr_production-lca']}), dim='Categories1')

    # Keep only food categories present in BOTH matrices, aligned order (the
    # multiply over Categories1 is positional).
    common_cat = sorted(set(dm_lca_world.col_labels['Categories1'])
    ...
    array_phys_world = dm_lca_world[:, :, 'agr_production-lca', :, np.newaxis] \
    ...
    DM_TCAF_lca['lca-world'].add(array_temp, dim='Variables', col_label='agr_production-tcaf', unit='CHF')

    # =====================================================================
    # Step Switzerland
    # =====================================================================
    dm_lca_ch = DM_livestock_to_TCAF['meat-ch'].filter({'Variables': ['agr_production-lca']})
    dm_lca_ch.append(DM_livestock_to_TCAF['asf-ch'].filter({'Variables': ['agr_production-lca']}), dim='Categories1')
    DM_landuse_to_TCAF['prod-ch'].switch_categories_order(cat1="Categories2", cat2="Categories1")
    dm_lca_ch.append(DM_landuse_to_TCAF['prod-ch'].filter({'Variables': ['agr_production-lca']}), dim='Categories1')

    # Align food (Categories1) and method (Categories2) on both matrices.
    common_food = sorted(set(dm_lca_ch.col_labels['Categories1'])
                         & set(DM_TCAF_lca['lca-switzerland'].col_labels['Categories1']))
    common_method = sorted(set(dm_lca_ch.col_labels['Categories2'])
                           & set(DM_TCAF_lca['lca-switzerland'].col_labels['Categories2']))
    common_years = sorted(set(dm_lca_ch.col_labels['Years'])
                          & set(DM_TCAF_lca['lca-switzerland'].col_labels['Years']))
    for dm in (dm_lca_ch, DM_TCAF_lca['lca-switzerland']):
        dm.filter({'Years': common_years, 'Categories1': common_food,
                   'Categories2': common_method}, inplace=True)
        dm.sort('Categories1')
        dm.sort('Categories2')
        dm.sort('Years')
    ...
    array_phys = dm_lca_ch[:, :, 'agr_production-lca', :, :, np.newaxis] \
    ...
    def _calib(dm_raw_single, cal_var):
        ...
        return calibration_rates(dm_raw_single, dmc, years_setting,
    ...
    if 'global-warming' in imp_ch:
        ...
        prod_asf, prod_crop, prod_total = _sum_asf_total(array_phys[:, :, :, :, i_gw], ref_ch)
        is_asf = np.array([f.startswith('meat-') or f.startswith('abp-') for f in food_ch])
        ...
        pop_asf = None
        dm_pop = (DM_livestock_to_TCAF.get('population-ch', None)
                  if isinstance(DM_livestock_to_TCAF, dict) else None)
        if (cdm_ghg_ef_perhead is not None and cdm_lsu_per_head is not None
            ...
            try:
                ...
                dm_pop = dm_pop.filter({'Country': keep_c, 'Years': keep_y}, inplace=False)
                ...
                dm_young = (DM_livestock_to_TCAF.get('bovine-young-share-ch', None)
                            if isinstance(DM_livestock_to_TCAF, dict) else None)
                if dm_young is not None:
                    dm_young = dm_young.filter({'Country': keep_c, 'Years': keep_y}, inplace=False)

                def _young_share(meth):
                    if dm_young is None or meth not in dm_young.col_labels['Categories1']:
                        return 0.0
                    ys = dm_young.array[:, :, dm_young.idx['bovine_young-share'], dm_young.idx[meth]]
                ...
                for fpos, food in enumerate(food_ch):
                    ...
                    for mpos, meth in enumerate(method_ch):
                        ...
                        if var not in dm_pop.col_labels['Variables']:
                            continue
                        head = dm_pop.array[:, :, dm_pop.idx[var], dm_pop.idx[food]] / lsu_ph
                        ...
                        for ipos, imp in enumerate(imp_ch):
                            if imp not in ef_vars:
                                continue
                            val = head_adult * cdm_ghg_ef_perhead.array[ex[imp], ex[food], ex[meth]]
                            if head_young is not None:
                                val = val + head_young * cdm_ghg_ef_perhead.array[
        ...
        dm_pop_asf = _cy_to_t_dm(pop_asf, ref_ch, 'ghg_asf_population_raw') if pop_asf is not None else None
        r_pop_asf = (_calib(dm_pop_asf, 'cal_tcaf_lca_ghg-emissions-livestock')
                     if dm_pop_asf is not None else None)

        if r_prod_asf is not None:
            cal_liv = dm_cal_ghg_lca.filter({'Variables': ['cal_tcaf_lca_ghg-emissions-livestock']})
            oy_liv = cal_liv.col_labels['Years']
            print("\n[TCAF LCA] STAGE 1 - livestock GHG vs inventory 3A+3B [t CO2-eq]:")
            print(f"    {'year':>6} {'observed':>12} {'prod-based':>12} {'x rate':>7} "
                  f"{'pop-based':>12} {'x rate':>7}")
            ci = 0
            for yi, yr in enumerate(years_ch):
                obs = cal_liv.array[ci, oy_liv.index(yr), 0] if yr in oy_liv else float('nan')
            ...
            def _dist(r):
                if r is None:
                    return None
                rr = np.array([r[ci, years_ch.index(y), 'cal_rate']
                               for y in oy_liv if y in years_ch])
            ...
            if d_pop is not None:
                ...
                print(f"    -> mean |rate-1|: production={d_prod:.3f}, population={d_pop:.3f} "
                      f"=> better match: {better}")
            else:
                print(f"    -> mean |rate-1|: production={d_prod:.3f} (population path unavailable)")
            ...
            if return_diagnostics:
                ...
                def _byyr(dm, israte):
                    if dm is None:
                        return None
                    return {yr: (float(dm[ci0, years_ch.index(yr), 'cal_rate']) if israte
                                 else float(dm.array[ci0, years_ch.index(yr), 0]))
                            for yr in years_ch}
                _diag['years'] = list(years_ch)
                _diag['stage1'] = {
                    'observed_3A3B': {yr: (float(cal_liv.array[ci0, oy_liv.index(yr), 0])
                                           if yr in oy_liv else float('nan')) for yr in years_ch},
                    ...
                    'd_prod': d_prod, 'd_pop': d_pop,
                    'better': ('production-based' if (d_pop is None or d_prod <= d_pop)
        ...
        dm_prod_total_in, r_prod_total = _stage1_total(prod_asf, r_prod_asf, 'ghg_total_production_stage1')
        dm_pop_total_in, r_pop_total = (_stage1_total(pop_asf, r_pop_asf, 'tcaf_lca_ghg-emissions_stage1')
                                        if pop_asf is not None else (None, None))

        # Driver: population if available, else production.
        if r_pop_asf is not None and r_pop_total is not None:
            drive_r1, drive_r2 = r_pop_asf[:, :, 'cal_rate'], r_pop_total[:, :, 'cal_rate']
            drive_total_in = dm_pop_total_in
        elif r_prod_asf is not None and r_prod_total is not None:
            drive_r1, drive_r2 = r_prod_asf[:, :, 'cal_rate'], r_prod_total[:, :, 'cal_rate']
        ...
        if drive_r1 is not None:
            ...
            array_phys[:, :, :, :, i_gw] = array_phys[:, :, :, :, i_gw] * rate1_food[:, :, :, np.newaxis]
            array_phys[:, :, :, :, i_gw] = array_phys[:, :, :, :, i_gw] * drive_r2[:, :, np.newaxis, np.newaxis]
            ...
            dm_cal_total = DataMatrix.based_on(
                ...
                format=ref_ch,
                change={'Variables': ['tcaf_lca_ghg-emissions'], 'Categories1': None,
                        'Categories2': None, 'Categories3': None},
                units={'tcaf_lca_ghg-emissions': 't CO2-eq'})
            dm_ghg_lca_ch.append(dm_cal_total, dim='Variables')

            cal_tot = dm_cal_ghg_lca.filter({'Variables': ['cal_tcaf_lca_ghg-emissions']})
            ...
            ci = 0
            for yi, yr in enumerate(years_ch):
                obs = cal_tot.array[ci, oy_tot.index(yr), 0] if yr in oy_tot else float('nan')
                if dm_prod_total_in is not None:
                    prod_str = (f"{dm_prod_total_in.array[ci, yi, 0]:>12,.0f} "
                                f"{r_prod_total[ci, yi, 'cal_rate']:>7.3f}")
                else:
                    prod_str = f"{'-':>12} {'-':>7}"
                if dm_pop_total_in is not None:
                    pop_str = (f"{dm_pop_total_in.array[ci, yi, 0]:>12,.0f} "
            ...
            if return_diagnostics:
                ...
                def _byyr2(dm, israte):
                    ...
                    return {yr: (float(dm[ci0, years_ch.index(yr), 'cal_rate']) if israte
                            ...
                            for yr in years_ch}
                _diag['stage2'] = {
                    'observed_cat3': {yr: (float(cal_tot.array[ci0, oy_tot.index(yr), 0])
                                           if yr in oy_tot else float('nan')) for yr in years_ch},
    ...
    DM_TCAF_lca['lca-switzerland'].add(array_temp, dim='Variables',
                                       col_label='agr_production-tcaf', unit='CHF')
...
def TCAF(lever_setting, years_setting, DM_input, interface=Interface()):
    ...
    DM_TCAF_lca, dm_ghg_lca_ch = TCAF_lca_workflow(
        DM_TCAF_lca, DM_crop_to_TCAF, DM_landuse_to_TCAF, DM_livestock_to_TCAF,
        CDM_const, CDM_MF, years_setting=years_setting, dm_cal_ghg_lca=dm_cal_ghg_lca,
        cdm_ghg_ef_perhead=cdm_ghg_ef_perhead, cdm_lsu_per_head=cdm_lsu_per_head)
...
_T5_SELECTORS = {
    'abp-dairy-milk':    {'intensive': 'cull cow, conventional, lowland milk system, silage maize 5 to 10%, at farm gate',
                          'organic':   'cull cow, organic, lowland milk system, silage maize 5 to 10%, at farm gate'},
    'meat-bovine':       {'intensive': 'beef cattle, conventional, national average, at farm gate',
                          'organic':   'beef cattle, organic, national average, at farm gate'},
    'meat-bovine-young': {'intensive': 'calf, 13 days old, conventional, lowland milk system, silage maize 5 to 10%, at farm gate',
                          'organic':   'calf, 13 days old, organic, lowland milk system, silage maize 5 to 10%, at farm gate'},
    'meat-pig':          {'intensive': 'pig, conventional, national average, at farm gate',
                          'organic':   'pig, organic, national average, at farm gate'},
    'meat-sheep':        {'intensive': 'lamb, conventional, indoor production system, at farm gate',
                          'organic':   'lamb, organic, system number 1, at farm gate'},
    'meat-poultry':      {'intensive': 'broiler, conventional, at farm gate',
                          'organic':   'broiler, organic, at farm gate'},
    'abp-hens-egg':      {'intensive': 'cull hen, conventional, national average, at farm gate',
                          'organic':   'cull hen, organic, at farm gate'},
    'meat-oth-animal':   {'intensive': 'kid goat, conventional, intensive forage area, at farm gate',
                          'organic':   None},
}
_T5_WEIGHT = {'abp-dairy-milk': 670.05, 'meat-bovine': 650.0, 'meat-bovine-young': 79.56,
              'meat-pig': 115.4, 'meat-sheep': 35.0, 'meat-poultry': 2.04,
              'abp-hens-egg': 1.9, 'meat-oth-animal': 9.0}
...
_MAIN_ANIMAL = [
    (r'cull cow|dairy cow',        [r'^cull cow$', r'cull dairy cow', r'dairy cow', r'cull cow']),
...
def _coproduct_db_weights(csv_path):
  ...
  for food, meth_sel in _T5_SELECTORS.items():
    ...
    for meth, sel in meth_sel.items():
      ...
      out[food][meth] = _main_weight(hit.iloc[0]['Process'], hit.iloc[0]['Summary']) \
...
def _print_calibration_comparison(a, b):
  ...
  print(f"{'year':>6} {'observed':>12} {'rawT5':>12} {'rawDB':>12} "
  ...
  for y in yrs:
    ...
    print(f"{y:>6} {o:>12,.0f} {rt:>12,.0f} {rd:>12,.0f} "
  ...
  print(f"closer to inventory (rate=1): "
...
def resolve_lcia_csv(explicit=None, filename='lcia_animal_production_recipe.csv',
                     start=None, max_up=6):
  ...
  rel_candidates = [
      ...
      os.path.join('backend', 'model', 'pre_processing', 'TCAF', 'data', 'data_pool', filename),
      filename,
  ]
  tried = []
  ...
  for base in starts:
    ...
    if repo_root is not None:
      break
  skip = {'.git', 'node_modules', '.venv', 'venv', 'env', '__pycache__', '.idea', '.mypy_cache'}
  if repo_root is not None:
    matches = []
  ...
  raise FileNotFoundError(
      ...
      + (f"\n  ...and os.walk('{repo_root}')" if repo_root
         else "\n  (no repo-root marker found to anchor a search)")
      + "\nPass csv_path='/abs/path/to/lcia_animal_production_recipe.csv' explicitly.")


def TCAF_ghg_calibration_weight_test(csv_path=None, years_setting=None, lever_setting=None):
  ...
  DM_input = filter_country_and_load_data_from_pickles(
      country_list=country_list, modules_list='TCAF', filter_country=False)['TCAF']
  ...
  if cdm_ef_t5 is None or cdm_lsu is None:
    raise RuntimeError("pickle lacks 'liv_ghg_ef_perhead' / 'liv_lsu_per_head'; "
  ...
  for food in _T5_WEIGHT:
    ...
    print(f"  {food:20} {_T5_WEIGHT[food]:>8} {_fmt(wi):>8} "
  ...
  def _run(cdm_ef, tag):
    ...
    _, _, diag = TCAF_lca_workflow(
        lca, crop, landuse, livestock, CDM_const, CDM_MF,
        years_setting=years_setting,
        dm_cal_ghg_lca=(dm_cal.copy() if dm_cal is not None else None),
        cdm_ghg_ef_perhead=cdm_ef, cdm_lsu_per_head=cdm_lsu,
