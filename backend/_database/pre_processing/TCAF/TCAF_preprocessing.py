...
def _load_lca_monetization_factors_chf(current_file_directory):
  ...
  """
  f = os.path.join(current_file_directory,
                   'data/monetization-factors/monetization-factor.csv')
  df = pd.read_csv(f)
  ...
  for recipe_cat, stem in _LCA_MF_CROSSWALK.items():
    if stem in eur_by_stem:
      ...
      print(f"  ⚠️ Monetization factor stem '{stem}' not found for "
...
def _select_monetizable_impacts(dm, mf_by_recipe):
  ...
  impact_dim = None
  for d in cat_dims:
    if set(dm.col_labels[d]) & set(mf_by_recipe):     # this axis carries ReCiPe impacts
      impact_dim = d
      break
  if impact_dim is None:
    raise ValueError("_select_monetizable_impacts: no axis matched the ReCiPe "
...
def _build_lca_mf_cdm(mf_by_recipe):
  ...
  cdm = ConstantDataMatrix(col_labels={'Variables': ['tcaf_mf_lca'],
                                       'Categories1': impacts})
...
def _keep_per_kg_rows(df, source_name):
  ...
  """
  unit = (df['Functional Unit'].astype(str)
  ...
  dropped = df[~keep]
  if len(dropped):
    print(f"  ℹ️ [{source_name}] dropping {len(dropped)} non-kg LCIA rows before "
          f"averaging: {dropped['Functional Unit'].value_counts().to_dict()}")
  n_missing = int(is_missing.sum())
  if n_missing:
    print(f"  ℹ️ [{source_name}] {n_missing} rows have no functional unit - kept "
...
def TCAF_lca_preprocessing():
  ...
  df_lcia_animal_production_recipe.loc[
    is_poultry & is_egg & ~is_flock, 'Category'] = 'avian-egg'
  # any poultry flock / breeding / cull-hen row -> excluded (not egg, not broiler)
  df_lcia_animal_production_recipe.loc[
    is_poultry & is_flock, 'Category'] = 'egg-system-bird'
  ...
  df_lcia_animal_production_recipe.loc[proc_lower.str.contains('cow milk', na=False),   'Category'] = 'dairy-milk'
  df_lcia_animal_production_recipe.loc[proc_lower.str.contains('goat milk', na=False),  'Category'] = 'milk-goat'
  df_lcia_animal_production_recipe.loc[proc_lower.str.contains('sheep milk', na=False), 'Category'] = 'milk-sheep'
  df_lcia_animal_production_recipe.loc[proc_lower.str.contains('cull goat', na=False),  'Category'] = 'caprine'
  ...
  df_lcia_animal_production_recipe['Category'] = (
    df_lcia_animal_production_recipe['Category']
  ...
  def _agg_prefer_explicit_kg(df, source_name):
    ...
    fu = (df['Functional Unit'].astype(str)
          .str.extract(r'\[([^\]]+)\]', expand=False).str.strip().str.lower())
    is_kg = fu.eq('kg')
    is_missing = df['Functional Unit'].isna() | fu.isna()
    kg_mean = (df[is_kg].groupby(grp, as_index=False)['Value'].mean()
               .rename(columns={'Value': 'val_kg'}))
    miss_mean = (df[is_missing].groupby(grp, as_index=False)['Value'].mean()
    ...
    if len(fell_back):
      combos = (fell_back[['Category', 'Production Method']]
                .drop_duplicates().itertuples(index=False, name=None))
      print(f"  ⚠️ [{source_name}] no explicit-kg LCIA rows; fell back to "
  ...
  mapping_lca = {
    ...
    'to-exclude': ['roughage', 'intercrops', 'nuts', 'seafood', 'fish',
                   'fish-market', 'fish-transformation', 'milk-goat', 'milk-sheep',
                   'others',              # plant 'others' (agave, seaweed, sawdust)
                   'vegetables-animal',   # stray vegetables in the animal file
                   'egg-system-bird']     # laying hens / pullets / breeding stock
  }
...
def TCAF_ghg_calibration_preprocessing():
  ...
  current_file_directory = os.path.dirname(os.path.abspath(__file__))
  f = os.path.join(current_file_directory,
                   'data/calibration/Tabelle-Inventar-2026_EN.xlsx')
  ...
  def _is_year(v):
    return (isinstance(v, (int, float, np.integer, np.floating))
            and not pd.isna(v) and float(v).is_integer()
            and 1900 < int(v) < 2100)
  hdr_i = next(i for i in df.index if _is_year(df.iat[i, 2]))
  year_cols = [(j, int(df.iat[hdr_i, j])) for j in range(df.shape[1])
  ...
  def _norm(x):
    ...
    return str(x).strip()
  agri_i = next(i for i in df.index
                if _norm(df.iat[i, 0]) == '3'
                and _norm(df.iat[i, 1]) == 'Agriculture')
  ...
  def _row_by_code(code, label_contains):
    return next(i for i in df.index
  ...
  vals_liv_mt = (np.array([float(df.iat[i_3A, j]) for j, _ in year_cols])
                 + np.array([float(df.iat[i_3B, j]) for j, _ in year_cols]))
  ...
  dm = DataMatrix(
      col_labels={'Country': ['Switzerland'], 'Years': years,
                  'Variables': ['cal_tcaf_lca_ghg-emissions',
                                'cal_tcaf_lca_ghg-emissions-livestock']},
      units={'cal_tcaf_lca_ghg-emissions': 'Mt CO2-eq',
             'cal_tcaf_lca_ghg-emissions-livestock': 'Mt CO2-eq'})
  dm.array = np.stack([vals_mt, vals_liv_mt], axis=-1)[np.newaxis, :, :]  # (1, nyear, 2)
  dm.change_unit('cal_tcaf_lca_ghg-emissions', 1e6,
                 old_unit='Mt CO2-eq', new_unit='t CO2-eq')
  dm.change_unit('cal_tcaf_lca_ghg-emissions-livestock', 1e6,
                 old_unit='Mt CO2-eq', new_unit='t CO2-eq')
  return dm
...
def TCAF_livestock_ghg_perhead_preprocessing():
  ...
  TABLE5 = {
    'abp-dairy-milk': {'weight': 670.05, 'life': 8, 'lsu': 0.7,
    ...
    'meat-bovine':    {'weight': 650.0, 'life': 2, 'lsu': 0.6,
    ...
    'meat-bovine-young': {'weight': 79.56, 'life': 1, 'lsu': 0.6,
    ...
    'meat-oth-animal': {'weight': 9.0, 'life': 1, 'lsu': 0.03,
  ...
  perkg['impact'] = (perkg['Impact category'].astype(str)
